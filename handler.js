...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('ig', async (data) => {
            ...
            data.reply(mess.wait) //https://api.chipa.xyz/api/download/ig?link=https://www.instagram.com/p/COUYR-MhAew/?utm_source=ig_web_copy_link&apikey=7DVG49XEXTX9CNASCQGX1CXZ
            getresult = await axios.get(`${configs.chipaUrl}/api/download/ig?link=${data.body}&apikey=${configs.ChipaKey}`)
            if(getresult.data.status == false) return data.reply(mess.error2)
            for(let i = 0; i < getresult.data.result.length; i++) {
                Client.sendFileFromUrl(data.from, getresult.data.result[i].data, `ig.${getresult.data.result[i].type}`, `「 INSTAGRAM 」\n\n*Username*: ${getresult.data.username}\n*Caption*: ${getresult.data.caption}`, data.message);
            }
        })
        ...
        Client.cmd.on('*', async (data) => {
            ...
            try {
            	...
            	break
                    ...
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, ${configs.janUrl}/api/nsfw/ass, 'p.jpg', mess.sange, message)
                    } catch {
...
`
                ...
                case 'ssweb':
                    ...
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, ${configs.dapUrl}/api/others/ssweb?url=${data.body}&apikey=${configs.DapKey}, 'p.jpg', mess.succes, message)
                    } catch {
                        data.reply(mess.error)
                    }
                    break

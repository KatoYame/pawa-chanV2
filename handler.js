...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('tiktok', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}ig [ link ]*\nContoh : ${data.prefix}tiktok https://vt.tiktok.com/ZSwWCk5o/`)
            data.reply(mess.wait) //https://api.lolhuman.xyz/api/tiktok?apikey=EpepBurik&url=https://vt.tiktok.com/ZSwWCk5o/
            getresult = await axios.get(`${configs.apiUrl}/api/tiktok?apikey=${configs.LolKey}&url=${data.body}`)
            if(getresult.data.status == false) return data.reply(getresult.data.message)
            caption = `*Tiktok Download*\n`
            caption += `*Title :* ${getresult.data.result.title}\n`
            caption += `*Keywords :* ${getresult.data.result.keywords}\n`
            caption += `*Description :* ${getresult.data.result.description}\n`
            caption += `*Duration :* ${getresult.data.result.durtaion}seconds\n`
            caption += `*Username :* ${getresult.data.result.author.username}\n`
            caption += `*Nickname :* ${getresult.data.result.author.nickname}\n`
            getVideo = getresult.data.result.link
                Client.sendFileFromUrl(data.from, getVideo, `ig.${getresult.data.result[i].type}`, caption, data.message);
        })
...
`
        ...
        Client.cmd.on('*', async (data) => {
            ...
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                data.reply(mess.wait)
                res = await axios.get(`${configs.DapUrl}/api/asupan/${data.command}?apikey=${configs.DapKey}`)
            	...
            	break
              	  ...
              	  if(!data.isGroup) return data.reply(mess.group)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    data.reply(mess.wait) //https://api.lolhuman.xyz/api/random/loli?apikey=EpepBurik
                    Client.sendFileFromUrl(from, `${configs.lolUrl}/api/random/loli?apikey=${configs.LolKey}`, 'loli.jpg', '```Bau� lolicon```', message)
                    } catch {
                        data.reply(mess.error2)
                    }
                    break
              	  ...
              	  if(!data.isGroup) return data.reply(mess.group)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    data.reply(mess.wait) //https://api.lolhuman.xyz/api/random/waifu?apikey=EpepBurik
					...
					case 'anime':
                    	if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ judul ]*\nContoh : ${data.prefix}${data.command} anime attack on titan`)
                        data.reply(mess.wait) //https://api.lolhuman.xyz/api/anime?apikey=EpepBurik&query=gotoubun%20no%20hanayome
...
`
                ...
                case 'spamsms':
                    ...
                    if (data.body == "6282248192917") return data.reply(`Mau di Banned?`)
                    data.reply(mess.wait)

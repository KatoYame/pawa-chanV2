...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('ig', async (data) => {
            ...
            data.reply(mess.wait) 
            getresult = await fetchJson(`${configs.lolUrl}/api/instagr2?apikey=${configs.LolKey}&url=${data.body}`)
            if(getresult.status == false) return data.reply(mess.error2)
            ini_result = getresult.data.result.media
                    for (var x of ini_result) {
                        ini_type = 'p.jpg'
                        if (x.includes(".mp4")) ini_type = 'video.mp4'
                        ini_buffer = await getBuffer(x)
                Client.sendFileFromUrl(data.from, ini_buffer, ini_type, `「 INSTAGRAM 」\n\n*Username*: ${getresult.result.data.account.username}\n*Caption*: ${getresult.data.result.caption}`, data.message);
                }
        })
            ...
            try {
            	...
            	break
                    ...
                    try {
                        ...
                        data.reply(mess.wait)
                        res = await fetchJson(`${configs.janUrl}/api/animek/kusonime?query=${data.body}`)
                        ttt = res.result
                        tod = `*「KUSONIME SEARCH」*\n*Hasil pencarian dari :* ${data.body}\n\nTitle : ${ttt.title}\nDesk : ${ttt.info}\nSinopsis : ${ttt.sinopsis}\nLink Download : ${ttt.link_dl}`
                        data.reply(tod)
                    } catch {
                    ...
                    try {
                        ...
                        data.reply(mess.wait) //
                        res = await fetchJson(`${configs.dapUrl}/api/anime/mal-name?name=${data.body}&apikey=${configs.DapKey}`)
                        ttt = res.result
                    ...
                    try {
                        ...
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ judul ]*\nContoh : ${data.prefix}anime attack on titan`)
                        data.reply(mess.wait) //
                        res = await fetchJson(`${configs.dapUrl}/api/anime/mal-name?name=${data.body}&apikey=${configs.zeksKey}`)
                        ttt = res.result
                        var teks = `*「MAL SEARCH」*\n\n*Hasil Pencarian : ${data.body}\n\n`
                        for(let i = 0; i < ttt.length; i++) {
                            teks += `*Title* : ${ttt[i].title}\n*Rating* : ${ttt[i].rating}\n*Url* : ${ttt[i].url}\n`
                ...
                try{
         			if(isLimit(data.sender)) return data.reply(mess.limit)
         			data.reply(mess.wait)
         			Client.sendFileFromUrl(from, `${lolUrl}/api/random2/${data.command}?apikey=${configs.LolKey}`, 'p.jpg', mess.sange, message)
         			} catch {
         	       ...
         	       case 'hentai4everyone':
         			...
         			if(isLimit(data.sender)) return data.reply(mess.limit)
         			data.reply(mess.wait)
         			Client.sendFileFromUrl(from, `${lolUrl}/api/random/nsfw/${data.command}?apikey=${configs.LolKey}`, 'p.jpg', mess.sange, message)
         			} catch {
         				data.reply(mess.error)
         			}
         			break
...
`
                ...
                case 'ssweb':
                    ...
                    data.reply(mess.wait) //https://api.lolhuman.xyz/api/sswebfull?apikey=EpepBurik&url=https://api.lolhuman.xyz
                    Client.sendFileFromUrl(from, `${configs.lolUrl}/api/sswebfull?apikey=${configs.LolKey}&url=${data.body}`, 'p.jpg', mess.succes, message)
                    } catch {
                        data.reply(mess.error)
                    }
                    break
        ...
        Client.handlerStick.on("*", async (datas) => {
            ...
            /*	Cara bikin stickercmd 
            	...
            	*/
            switch(idStick) {
            	case '2.5453414541226133e+123': //ping - speed
					const processsTime = (timestamp, now) => {
                return moment.duration(now - moment(timestamp * 1000)).asSeconds()
                }
                data.reply(`Speed : ${processsTime(message.messageTimestamp.low, moment())} _second_`)
					break
				case '2.652157292261574e+123': //runtime
					data.reply(`Runtime : _${kyun(process.uptime())}_`)
					break

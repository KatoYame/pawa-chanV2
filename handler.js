            ...
            try {
            	...
            	break
                    ...
                    case 'kusonime':
                        ...
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.apiUrl}/api/happymod?apikey=${configs.LolKey}&q=${data.body}`)
                        ttt = res.data.result
                        var teks = `*「 KUSONIME 」*\n\n`
                        teks += `*Title :* ${ttt.title}\n`
                        teks += `*Japanese :* ${ttt.japanese}\n`
                        teks += `*Gender:v :* ${ttt.genre}\n`
                        teks += `*Seasons :* ${ttt.seasons}\n`
                        teks += `*Producers :* ${ttt.producers}\n`
                        teks += `*Type :* ${ttt.type}\n`
                        teks += `*Total Eps :* ${ttt.total_episode}\n`
                        teks += `*Score :* ${ttt.score}\n`
                        teks += `*Duration :* ${ttt.duration}\n`
                        teks += `*Released On :* ${ttt.released_on}\n\n\n`
                        teks += `*↓Download Animenya disini ↓*\n\n`
                        link_dl = ttt.link_dl
                ...
                case 'hartatahta':
                    ...
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${command} [ query ]*\nContoh : ${data.prefix}${command} pawa chan`)
                        data.reply(mess.wait) //https://lolhuman.herokuapp.com/api/gsearch?apikey=EpepBurik&query=tahu

        ...
        Client.cmd.on('*', async (data) => {
            ...
            try {
            	...
            	break
                    ...
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}kusonime [ judul anime ]*\nContoh : ${data.prefix}kusonime bakemonogatari`)
                        data.reply(mess.wait)
                        ...
                        thumbnail = await getBuffer(res.data.result.thumb)
                        Client.sendFileFromUrl(from, thumbnail, 'p.jpg', te, message)
                    } catch {
                    ...
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    data.reply(mess.wait)
                    res = await getBuffer(`${configs.dapUrl}/api/nsfw/${data.command}?apikey=${configs.DapKey}`)
                    Client.sendFileFromUrl(from, res, 'p.jpg', mess.sange, message)
                    } catch {

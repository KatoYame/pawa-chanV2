...
const isNsfw = data.isGroup ? nsfw.includes(from) : false
*/
    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                Client.sendFileFromUrl(data.from, `${ytm.link}`, `${ytm.title} - Download.mp4`, `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
            } catch {
        ...
        Client.cmd.on('playvid', async (data) => {
            try {
                ...
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Title* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : mp4\n*Source* : ${ytm.source}\n*Link* : ${ytm.link}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.link}`, 'video.mp4', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
            } catch (e) {
                data.reply(mess.error2)
            }
        })
		...
		Client.cmd.on('antilink', (data) => {
            ...
            if(data.args[0].toLowerCase() == 'on') {
                ...
                fs.writeFileSync('./lib/json/dataGc.json', JSON.stringify(dataGc))
                data.reply('Antilink On\n*「❗」 Peringatan! antilink telah aktif jangan menyebarkan link group atau kamu akan di kik*')
            } else if(data.args[0].toLowerCase() == 'off') {
...
`
        ...
        Client.cmd.on('missing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'missing.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
                ...
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/missing-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}&text1=${text[0]}&text2=${text[1]}&text3=${text[2]}`, 'missing.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
        ...
        Client.cmd.on('calender', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'p.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
            } else if(data.mentionedJidList.length > 0) {
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/calender?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
        ...
        Client.cmd.on('removebg', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/removebg?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}calender`)
        ...
        Client.cmd.on('drawing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/draw-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
        ...
        Client.cmd.on('sketch', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/sketch-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}drawing`)
        ...
        Client.cmd.on('*', async (data) => {
            ...
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                data.reply(mess.wait)
                res = await axios.get(`${configs.DapUrl}/api/asupan/${command}?apikey=${configs.DapKey}`)
                supan = res.data.result
                Client.sendFileFromUrl(data.from, supan, 'video.mp4', `nih asupan nya @${data.sender.split('@')[0]}`, data.message)
            } catch (e) {
                data.reply(mess.error2)
            }
            	break
                    ...
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ judul anime ]*\nContoh : ${data.prefix}kusonime bakemonogatari`)
                        data.reply(mess.wait)
                        res = await axios.get(`${configs.janUrl}/api/animek/kusonime?query=${data.body}`)
                        tod = `*「KUSONIME SEARCH」*\n*Hasil pencarian dari :* ${data.body}\n\nTitle : ${res.data.result.title}\nDesk : ${res.data.result.info}\nSinopsis : ${res.data.result.sinopsis}\nLink Download : ${res.data.result.link_dl}`
                        data.reply(tod)
                    } catch (e) {
                        data.reply(`Maaf anime ${data.body} tidak ditemukan`)
                    }
                    break
					case 'anime':
                    try {
                        if(isLimit(data.sender)) return data.reply(mess.limit)
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ judul ]*\nContoh : ${data.prefix}anime attack on titan`)
                        data.reply(mess.wait) //
                        res = await axios.get(`${configs.dapUrl}/api/anime/mal-name?name=${data.body}&apikey=${configs.DapKey}`)
                        ttt = res.data.result
                        var teks = `*「MAL」*\n\n*Hasil Pencarian : ${data.body}*\n\n*Title :* ${ttt.title}\n*id :* ${ttt.id}\n*Detail Link :* ${ttt.detailsLink}\n*Episode Link :* ${ttt.episodesLink}\n*Type :* ${ttt.type}\n*Episodes :* ${ttt.episodes}\n*Status :* ${ttt.status}\n*Aired :* ${ttt.aired}\n`
                        for(let i = 0; i < ttt.length; i++) {
                            teks += `*Alternative Title* : ${ttt.alternativeTitles.Japanese[i]} ${ttt.alternativeTitles.English[i]} ${ttt.alternativeTitles.synoynms[i]}\n*Genre :* ${ttt.genres[i]}\n*Studio :* ${ttt.studios[i]}`
                        }
                        data.reply(teks)
                    } catch(e) {
                        data.reply(`Maaf anime ${data.body} tidak ditemukan`)
                    ...
                    try {
                        ...
                        if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ judul ]*\nContoh : ${data.prefix}anime attack on titan`)
                        data.reply(mess.wait) //
                        res = await axios.get(`${configs.dapUrl}/api/anime/mal-name?name=${data.body}&apikey=${configs.zeksKey}`)
                        ttt = res.data.result
                        var teks = `*「MAL SEARCH」*\n\n*Hasil Pencarian : ${data.body}\n\n`
                        for(let i = 0; i < ttt.length; i++) {
                            teks += `*Title* : ${ttt[i].title}\n*Rating* : ${ttt[i].rating}\n*Url* : ${ttt[i].url}\n`
        			...
        			if(!data.isGroup) return data.reply(mess.group)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    data.reply(mess.wait) //https://janbot-api.herokuapp.com/api/nsfw/ass
                    res = await axios.get(`${configs.janUrl}/api/nsfw/ass`)
                    ...
                    case 'armpits':
				    try{
					if(data.isGroup) return data.reply(mess.ungroup)
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    ...
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(!isPrem(data.sender)) return data.reply(mess.prem)
                    res = await axios.get(`https://meme-api.herokuapp.com/gimme/${command}`)
                    image = res.data.url
                    Client.sendFileFromUrl(from, image, 'p.jpg', mess.sange, message)
                    } catch {
                        data.reply(mess.error)
                ...
                case 'nulis':
                    ...
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?text=${data.body}&apikey=${configs.zeksKey}`, 'gambar.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
                    break
                ...
                case 'watercolour':
                    ...
                    p = data.body
                    text = p.split('|')
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?apikey=${configs.zeksKey}&text1=${text[0]}&text2=${text[1]}`, 'p.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
					break
                ...
                case 'hartatahta':
                    ...
                    if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ teks ]*\nContoh : ${data.prefix}${data.command} shiro`)
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/hartatahta?text=${data.body}&apikey=${configs.zeksKey}`, 'harta.jpg', `${mess.succes} @${data.sender.split('@')[0]}`, message)
...
`
                ...
                case 'ssweb':
                    ...
                    data.reply(mess.wait) //
                    image = await axios.get(`${configs.dapUrl}/api/others/ssweb?url=${data.body}&apikey=${configs.DapKey}`)
                    Client.sendFileFromUrl(from, image, 'p.jpg', mess.succes, message)
                    } catch {
                        data.reply(mess.error)
                    }
                    break

...
const starts = async (sesName) => {
    try {
		...
		client.on('CB:Call', json => {
			...
			setTimeout(async () =>{
				Client.reply(from, `❗ *Auto block system* ❗\nplease dont call, Chat owner untuk membuka blocked`, message)
			if (Client.blocklist.includes(json[1].from)) return
			if(isOwner) return data.reply(`Kalo buat owner mah apa aja`)
        ...
        client.on('new-msg', (message) => {
			...
			if (isGroup && !message.isAdmin && dataGc[from].antilink && /chat\.whatsapp\.com/gi.test(body)){
				...
				if(isAdmin) return data.reply(`Suka-suka admint lah`)
				if(isOwner) return data.reply(`Kalo buat owner mah apa aja`)
				if(body.match(`${groupInviteCode}`)) return Client.reply(from, `untuk link Group ini gpp lah ya`, message)

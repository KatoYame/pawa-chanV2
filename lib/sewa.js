...
const addSewaGroup = (gid, expired, _dir) => {
    ...
    _dir.push(obj)
    fs.writeFileSync('./lib/json/sewa.json', JSON.stringify(_dir))
}
 ...
 * @param {Object} _dir 
 */
const expiredCheck = (client, Client, _dir) => {
    setInterval(() => {
        let position = null
        Object.keys(_dir).forEach((i) => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                Client.sendText(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`)
                .then(() => {

...
│ *${p+=1}.* ${prefix}spatrick
│ *${p+=1}.* ${prefix}stickeranjing
│ *${p+=1}.* ${prefix}sgawrgura
...
│ *${p+=1}.* ${prefix}sewa _add/del_ _time_
│ *${p+=1}.* ${prefix}sewalist
│ *${p+=1}.* ${prefix}sewacheck
...
│donate, pensi ngebot aelah :'(
└────────────────────



ＴＨＸㅤＴＯ:
[ Myself, My God, My Parents,
DappaUhuy, Zeks, Arga(pensi),
RamlanID,  Elios&Ben(sc ori), Fadli,
LolHuman, Xteam, LeysCoders,
Akira, Zakky, Fboy, Lord Yudha, aqulz ]
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
*_Thanks For All Penyedia Rest Api_*
								&
...
const snk = `*「SnK 4 All User Bot」*
❏ Dilarang keras VCALL/CALL (Soft Block)
❏ Dilarang keras spam command! (Soft Block)
❏ Dilarang chat owner kalau g penting👊🏿 (Hard Block)
❏ Dilarang mengirim virus, slayer, bugTroli dsb. (Permanent Block)
❏ Jika menemukan error atau bug, silahkan report ke owner
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
*「SnK 4 User Prem/Sewa」*
❏ Jika bot di kick dari gc, join ulang 5k
❏ Jangan kirim BugGc (Bot akan leave & masa sewa dianggap habis)
❏ Buat yg udh beli premium dri lama, add back ke gc permanent(SnK berlaku)

Note:
_❏ Jika tidak sengaja call/vcall bot chat owner untuk membuka block_
_❏ Bot tidak on 24 jam_
_❏ Pihak Bot tdk bertanggung jwb atas apa yg anda command pada bot_
_❏ Owner sesekali memantau aktifitas bot_
*_❏ SnK dpt berubah sewaktu-waktu_*
©Khaelsan`

const hargaprem = `╭─「Price List」
│
├❏Sewa Bot join ke group
│└•seminggu => 5k
│ └•sebulan => 7k
│  └•permanent => 12k
├───────────────
...
│pembayaran ke :  wa.me/6282248192917
└──「パ ワ ー」
...
│ └•permanentPrem = 1bulan
│  └•monthlyPrem = 2minggu
│
└──「パ ワ ー」

minat/mau tanya² chat owner
[ tekan button di bawah untuk kontak owner ]
